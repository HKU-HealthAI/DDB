...
in_filename=""
target_resolution=256 # for edit/mmu_single_image/mmu_multi_image, target resolution is setted in script
edit_type="edit" # t2i, edit, mmu_single_image, mmu_multi_image
base_name=${edit_type}_$(basename "$in_filename")
main_name="${base_name%.*}"
log_dir="./pre_token/${main_name}-${target_resolution}_log_center"
out_dir="./pre_token/${main_name}_vae_code-${target_resolution}_center"
mkdir -p "$log_dir"
for i in {0..31}
do
  gpu_id=$((i % 4 +4))
