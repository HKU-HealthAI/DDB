...
def center_crop(pil_image, crop_size):
    """True center crop with resize-to-fit behavior.

    The function resizes the image to ensure the requested crop size fits,
    then selects the exact center window of the target dimensions.
    """
    # 1. 保持原有的降采样逻辑（为了效率处理大图）
    while pil_image.size[0] >= 2 * crop_size[0] and pil_image.size[1] >= 2 * crop_size[1]:
        pil_image = pil_image.resize(tuple(x // 2 for x in pil_image.size), resample=Image.BOX)

    # 2. 保持原有的缩放逻辑（确保图像至少能填满目标尺寸）
    scale = max(crop_size[0] / pil_image.size[0], crop_size[1] / pil_image.size[1])
    pil_image = pil_image.resize(tuple(round(x * scale) for x in pil_image.size), resample=Image.BICUBIC)

    # 3. 修改此处：计算中心偏移量 (Center Offsets)
    # 原代码使用 random.randint，改为计算 (图像宽 - 目标宽) // 2
